...
async def upload_fileobj_to_greenfield(file: BytesIO, object_name: str, config: GreenfieldConfig):
    try:
        ...
        response = requests.post(
            ...
            data={"bucketName": config.bucket_name, "storageSource": "greenfield", "objectName": object_name},
...
async def upload_fileobj_to_walrus(file: BytesIO, object_name: str, config: GreenfieldConfig):
    try:
        file.seek(0)
        logger.info(f"Uploading file {object_name} to {config.url}:{config.bucket_name}/{object_name}")
        response = requests.post(
            f"{config.url}/api/v1/walrus/object",
            data={"bucketName": config.bucket_name, "storageSource": "walrus", "objectName": object_name},
            files={"file": (object_name, file, "image/png")},
            headers={"x-api-key": config.apikey},
        )
